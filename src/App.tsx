...
import { ArrowRight, ChevronRight, ChevronLeft, Check, Clock, Calendar, Users, Briefcase, Wine, Coffee, Music, Monitor, Minus, Plus, Sparkles, Sun, Moon, Sunrise, Star, Utensils, Wifi, Gift, Palette, LayoutTemplate, Droplets, Map, Mail, Loader, Send, FileText, Share2 } from 'lucide-react';
...
export default function App() {
  ...
  const handleSubmit = (e) => {
    ...
    Promise.all([sendAdmin, sendClient]).then(() => {
          fetch('https://api.counterapi.dev/v1/lmeb-immersive/leads/up').catch(console.error);
          setIsSending(false); setIsSent(true);
      }).catch((error) => { console.error("Erreur d'envoi", error); setIsSending(false); alert("Erreur d'envoi."); });
  };
    ...
    return (
      <div className="relative h-[100dvh] w-full bg-[#050505] overflow-hidden flex flex-col items-center justify-center p-6 text-white print:hidden">
           ...
           <div className="relative z-10 max-w-xl w-full bg-black/40 backdrop-blur-xl border border-white/10 p-12 md:p-16 flex flex-col items-center text-center shadow-2xl animate-fade-in-up">
              <div className="mb-8 relative"><div className="absolute inset-0 bg-amber-600 blur-xl opacity-20 animate-pulse"></div><div className="w-20 h-20 rounded-full border border-amber-600/50 flex items-center justify-center bg-black/50 relative z-10"><Check size={32} className="text-amber-500" /></div></div>
              <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-amber-600 mb-4">Confirmation</div>
              <h2 className="text-4xl md:text-5xl font-serif mb-6 text-transparent bg-clip-text bg-gradient-to-b from-white to-neutral-400">Demande Transmise</h2>
              <div className="w-12 h-px bg-white/20 mb-8"></div>
              <p className="font-light text-neutral-300 leading-relaxed text-sm mb-10">Merci <span className="text-white font-medium">{data.contact.name}</span>.<br/>Votre vision a été capturée. Une confirmation vient d'être envoyée à votre adresse email.<br/><br/><span className="text-xs font-mono text-neutral-500">Nous reviendrons vers vous sous 24h avec une proposition chiffrée.</span></p>
              <div className="flex flex-col gap-4 w-full md:w-auto items-center">
                  <button onClick={() => window.location.reload()} className="group relative px-8 py-4 bg-white/5 border border-white/10 hover:border-amber-600/50 transition-all duration-500 cursor-pointer w-full"><div className="absolute inset-0 bg-amber-600/10 scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500 ease-out"></div><span className="relative font-mono text-[10px] uppercase tracking-[0.2em] text-white group-hover:text-amber-500 transition-colors">Retour à l'accueil</span></button>
                  <button onClick={handleSaveTheDate} className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.25em] text-neutral-500 hover:text-white transition-colors border-b border-transparent hover:border-white pb-1"><Share2 size={10} /> Copier l'invitation</button>
                  <a href="https://www.lemonde-enbouteille.be/salle" target="_blank" rel="noopener noreferrer" className="font-mono text-[9px] uppercase tracking-[0.25em] text-neutral-500 hover:text-white transition-colors border-b border-white/10 hover:border-white pb-1">Voir le lieu officiel</a>
              </div>
           </div>
      </div>
    )
      ...
      <div className="relative h-[100dvh] w-full bg-[#050505] flex flex-col items-center justify-center p-6 md:p-8 text-white overflow-hidden print:hidden">
        ...
        <div className="relative z-10 text-center w-full max-w-4xl mx-auto flex flex-col items-center justify-center h-full">
          ...
          <div className="animate-fade-in-up cursor-default select-none" onClick={handleLogoClick}>
            <img src="https://www.lemonde-enbouteille.be/web/image/26768-edef09a5/LOGO%20l%27immersive-24.png" alt="Logo L'Immersive" className="w-28 md:w-40 mx-auto mb-4 md:mb-6 opacity-90 drop-shadow-2xl" />
            <h1 className="text-5xl md:text-7xl lg:text-[7rem] font-serif tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white to-neutral-500 leading-none mb-3">L'IMMERSIVE</h1>
            <p className="font-mono text-[9px] md:text-xs uppercase tracking-[0.4em] md:tracking-[0.6em] text-amber-500 mb-8 md:mb-10">Le Monde en Bouteille</p>
          </div>
          <div className="border-l border-amber-600/30 pl-6 text-left max-w-lg mx-auto backdrop-blur-sm py-2 mb-10 md:mb-12 animate-fade-in-up delay-200">
             <p className="text-neutral-400 font-light leading-relaxed text-xs md:text-sm">Une adresse confidentielle à Namur. <br className="hidden md:block"/><strong>Un espace événementiel privatif alliant architecture de caractère et équipements connectés.</strong> <br className="hidden md:block"/>Réunions, séminaires ou soirées : ne louez pas une salle, vivez une expérience.</p>
          </div>
          <button onClick={() => goToStep(1)} className="group relative px-8 py-4 md:px-10 md:py-5 bg-white/5 border border-white/10 hover:border-amber-600/50 transition-all duration-500 w-full md:w-auto cursor-pointer z-50 animate-fade-in-up delay-300">
          ...
          <div className="mt-12 md:mt-16 animate-fade-in-up delay-500 flex flex-col items-center gap-3 opacity-60 hover:opacity-100 transition-opacity duration-500">
             <div className="flex items-center gap-3"><div className="h-px w-8 bg-white/20"></div><p className="font-mono text-[9px] uppercase tracking-[0.2em] text-neutral-400">Confiance : Entreprises & Privés</p><div className="h-px w-8 bg-white/20"></div></div>
             <a href="https://www.lemonde-enbouteille.be/salle" target="_blank" rel="noopener noreferrer" className="font-mono text-[9px] text-neutral-500 hover:text-amber-500 uppercase tracking-widest transition-colors">www.lemonde-enbouteille.be</a>
          </div>
          ...
          {showAdmin && (
            <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center animate-fade-in-up" onClick={() => setShowAdmin(false)}>
               <div className="border border-white/10 p-8 w-80 text-center shadow-2xl relative bg-[#0a0a0a]" onClick={e => e.stopPropagation()}>
                  <div className="text-amber-600 font-mono text-xs uppercase tracking-widest mb-8 flex items-center justify-center gap-2">DATA FLUX</div>
                  <div className="space-y-4">
                      <div className="flex justify-between items-center p-3 bg-white/5 border border-white/5"><div className="text-[10px] text-neutral-500 uppercase tracking-wider">Trafic</div><div className="text-xl font-serif text-white">{stats.visits}</div></div>
                      <div className="flex justify-between items-center p-3 bg-white/5 border border-white/5"><div className="text-[10px] text-neutral-400 uppercase tracking-wider">Final Step</div><div className="text-xl font-serif text-white">{stats.finalStep}</div></div>
                      <div className="flex justify-between items-center p-3 bg-amber-900/10 border border-amber-600/30"><div className="text-[10px] text-amber-600 uppercase tracking-wider font-bold">Devis Reçus</div><div className="text-xl font-serif text-amber-500">{stats.leads}</div></div>
                  ...
                  <div className="mt-6 pt-6 border-t border-white/10 text-[10px] text-neutral-600 font-mono">Taux de conversion : {stats.visits > 0 ? ((stats.leads / stats.visits) * 100).toFixed(1) : 0}%</div>
               </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="relative h-[100dvh] w-full bg-[#080808] text-white overflow-hidden font-sans flex flex-col md:flex-row print:bg-white print:text-black print:h-auto print:overflow-visible">
      ...
      <div className="hidden print:block p-8 w-full max-w-4xl mx-auto">
          <div className="flex justify-between items-start mb-12 border-b border-black pb-8"><div><h1 className="text-4xl font-serif text-black mb-2">L'IMMERSIVE</h1><p className="font-mono text-xs uppercase tracking-widest text-neutral-600">Le Monde en Bouteille, Namur</p></div><div className="text-right"><div className="font-mono text-sm text-neutral-500">Pré-réservation</div><div className="text-xl font-bold">{data.date}</div></div></div>
          <div className="mb-8"><h3 className="font-mono text-xs uppercase tracking-widest text-neutral-500 mb-4">Client</h3><div className="text-lg">{data.contact.name}</div><div className="text-sm text-neutral-600">{data.contact.email}</div><div className="text-sm text-neutral-600">{data.contact.phone}</div></div>
          <div className="space-y-4 border-t border-neutral-200 pt-8 mb-12">
              <div className="flex justify-between"><span>Location ({data.timeSlot?.title})</span><span>{data.timeSlot?.price} €</span></div>
              {!isDryHire && data.experience && (<div className="flex justify-between"><span>Expérience : {data.experience.title} ({data.pax} pers.)</span><span>{data.experience.price === -1 ? 'Sur devis' : `${data.experience.price * data.pax} €`}</span></div>)}
              {data.selectedServices.map(id => {
                  ...
                  return (<div key={id} className="flex justify-between text-neutral-600"><span>+ {srv.title}</span><span>{price} €</span></div>);
              })}
          </div>
          <div className="border-t-2 border-black pt-8 flex justify-between items-center text-2xl font-serif"><span>Total Estimé</span><span>{totalAmount} € {isCustom && '+ Devis'}</span></div>
          <div className="mt-16 text-center text-xs font-mono text-neutral-400">Document généré automatiquement le {new Date().toLocaleDateString()}</div>
      </div>
        ...
        {step === 2 && (
          <div className="flex-1 p-6 md:p-16 flex flex-col justify-center animate-fade-in-right duration-500 overflow-y-auto">
             <div className="max-w-7xl mx-auto w-full h-full flex flex-col">
               <div className="mb-6 md:mb-8 flex items-end justify-between border-b border-white/10 pb-6 mt-16 md:mt-0"><div><span className="text-amber-600 font-mono text-xs uppercase tracking-widest mb-2 block">02 — Intention</span><h2 className="text-3xl md:text-4xl font-serif text-white">L'Énergie du Moment</h2></div></div>
          ...
          <div className="flex-1 flex flex-col justify-center p-6 md:p-16 animate-fade-in-right duration-500 overflow-y-auto">
             <div className="max-w-7xl mx-auto w-full h-full flex flex-col">
               ...
               <div className="flex-1 flex flex-col gap-4 md:gap-6 pb-20 md:pb-0">
                  {FORMATS.map((fmt) => (
                    <button key={fmt.id} onClick={() => { setData({...data, format: fmt}); autoNext(4); }} className="group relative flex-1 min-h-[120px] md:min-h-0 border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] transition-all duration-300 flex items-center overflow-hidden hover:border-amber-600/30">
                       ...
                       <div className="flex-1 p-6 md:p-8 text-left flex items-center justify-between">
                          <div><h3 className="text-xl md:text-3xl font-serif text-white mb-1 md:mb-2 group-hover:text-amber-500 transition-colors">{fmt.title}</h3><div className="flex items-center gap-4 text-[10px] md:text-xs font-mono uppercase tracking-widest text-neutral-500"><span>{fmt.subtitle}</span></div></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
                   ...
                   <div className="flex flex-col items-center gap-6 p-8 md:p-10 border border-white/10 bg-white/[0.02] relative">
                        ...
                        <div className="w-full flex flex-col gap-4">
                          <input type="date" min={new Date().toISOString().split('T')[0]} onChange={(e) => setData({...data, date: e.target.value})} className="bg-transparent text-xl md:text-2xl text-center font-serif text-white w-full outline-none border-b border-white/10 pb-2 focus:border-amber-600 transition-colors uppercase [color-scheme:dark]" />
                          {isMultiDay && (<input type="date" min={data.date || new Date().toISOString().split('T')[0]} onChange={(e) => setData({...data, endDate: e.target.value})} className="bg-transparent text-xl md:text-2xl text-center font-serif text-amber-500 w-full outline-none border-b border-white/10 pb-2 focus:border-amber-600 transition-colors uppercase [color-scheme:dark] animate-fade-in-up" />)}
                        </div>
             ...
             <div className="w-full max-w-5xl flex flex-col md:flex-row bg-[#0a0a0a]/95 md:bg-[#0a0a0a]/90 border-0 md:border border-white/10 backdrop-blur-xl shadow-2xl relative z-10 rounded-none md:rounded-sm overflow-hidden h-full md:h-[85vh]">
                <div className="w-full md:w-1/2 p-6 md:p-10 border-b md:border-b-0 md:border-r border-white/10 flex flex-col relative overflow-y-auto scrollbar-hide pt-20 md:pt-10 max-h-[50vh] md:max-h-full">
                   ...
                   <div className="pt-8 mt-12 border-t border-white/10 text-center pb-8 md:pb-0">
                      ...
                      <button onClick={handlePrint} className="mt-6 flex items-center justify-center gap-2 w-full text-[10px] uppercase tracking-widest text-neutral-500 hover:text-white transition-colors"><FileText size={12} /> Télécharger le devis officiel</button>
                   </div>
                </div>
